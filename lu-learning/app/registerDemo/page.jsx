...
const page = () => {
    ...
    const GOOGLE_FORM_URL = 'YOUR_GOOGLE_FORM_URL_HERE'; // Replace with your Google Form URL
const GOOGLE_FORM_ACTION_URL = 'https://forms.gle/GabqhHqdm9nvifky9'; // Replace with your Google Form action URL

const handleSubmit = async (event) => {
  event.preventDefault();

        // Create a FormData object to send to Google Forms
        const googleFormData = new FormData();
        googleFormData.append('entry.field1', formData.name);
        googleFormData.append('entry.field2', formData.email);
        googleFormData.append('entry.field3', formData.age);
        googleFormData.append('entry.field4', formData.gender);
        googleFormData.append('entry.field5', formData.whatsapp);
        googleFormData.append('entry.field6', formData.selectedCourse);

        // Send the data to Google Forms
        try {
            await axios.post(GOOGLE_FORM_ACTION_URL, googleFormData, {
                headers: {
    ...
    return (
        <div id="reg" className="bg-lightkblue" style={{ background: "linear-gradient(to bottom, #DCDDEc, #C3E1FD)" }}>
            ...
            <form id="registrationForm" onSubmit={handleSubmit}>
            {/* Hidden input fields for Google Forms questions */}
            <input type="hidden" name="entry.field1" value={formData.name} />
            <input type="hidden" name="entry.field2" value={formData.email} />
            <input type="hidden" name="entry.field3" value={formData.age} />
            <input type="hidden" name="entry.field4" value={formData.gender} />
            <input type="hidden" name="entry.field5" value={formData.whatsapp} />
            <input type="hidden" name="entry.field6" value={formData.selectedCourse} />
